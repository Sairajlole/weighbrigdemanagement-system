...
template <typename T>
std::function<void(T)> WrapResult(std::function<void(T)> result) {
  return [result = std::move(result)](T value) mutable {
    auto shared_value = std::make_shared<T>(std::move(value));
    auto shared_result = std::make_shared<std::function<void(T)>>(std::move(result));
    RunOnPlatformThread([shared_result, shared_value]() {
      (*shared_result)(std::move(*shared_value));
    });
  };
}
// ─── End Platform Thread Dispatch ───────────────────────────────────────────
...
void CloudFirestorePlugin::NamedQueryGet(
  ...
  result = WrapResult(std::move(result));
  Firestore* firestore = GetFirestoreFromPigeon(app);
        ...
        [this, serverTimestampBehavior = serverTimestampBehavior_,
         metadataChanges](const firebase::firestore::QuerySnapshot& snapshot,
          ...
          if (error == firebase::firestore::kErrorOk) {
            ...
            RunOnPlatformThread([this, toListResult = std::move(toListResult)]() {
            ...
            RunOnPlatformThread([this, errorMessage, details = std::move(details)]() {
    ...
    listener_ = reference_->AddSnapshotListener(
        ...
        [this, serverTimestampBehavior = serverTimestampBehavior_,
         metadataChanges](const firebase::firestore::DocumentSnapshot& snapshot,
                          ...
                          const std::string& errorMessage) mutable {
          if (error == firebase::firestore::kErrorOk) {
            auto encoded = ParseDocumentSnapshot(snapshot, serverTimestampBehavior)
                    .ToEncodableList();
            RunOnPlatformThread([this, encoded = std::move(encoded)]() {
              events_->Success(encoded);
            });
          } else {
            EncodableMap details;
            details[EncodableValue("code")] =
